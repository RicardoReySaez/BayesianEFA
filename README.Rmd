...
The `BayesEFA` package provides a straightforward way to estimate Bayesian Exploratory Factor Analysis (EFA) models via `Stan`. 
...
Since `BayesEFA` includes custom C++ functions, macOS users must configure the R compilation toolchain. Following the recommendations from the [latent](https://github.com/Marcosjnez/latent) package, we suggest using `macrtools` by James Balamuta to automate this setup:
